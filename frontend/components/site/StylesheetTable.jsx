...
const StylesheetTableDiv = styled.tbody`
  td {
    & > div {
      max-width: 100%;
      display: block;
...
const StylesheetTable = props => {
  ...
  const { data: linkDetail, error: linkDetailError } = useSWR(
    () =>
      query.siteId
        ? `/api/site/${query.siteId}/scan/${props.val.scan_id}/stylesheet/${props.val.id}/`
        : null,
    fetcher,
    {
  ...
  if (!linkDetail || !user) {
    return (
      <Fragment>
        <StylesheetTableDiv className={`bg-white`}>
          <tr>
          ...
          <div className={`flex items-center`}>
            <div>
              ...
              <div className={`flex justify-start inline-text-sm leading-5 text-gray-500`}>
                <Link href="/dashboard/site/[siteId]/stylesheets/[stylesheetId]/details" as={`/dashboard/site/${query.siteId}/stylesheets/${linkDetail.id}/details`}>
...
export default StylesheetTable

StylesheetTable.propTypes = {}
