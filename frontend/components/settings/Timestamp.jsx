...
const TimestampSettings = () => {
	...
	const { data: user, mutate: updateUser } = useSWR(
		() => '/api/auth/user/',
							...
							<div>
								<div>
									<h3 className={`text-lg leading-6 font-medium text-gray-900`}>
										{GlobalLabels[0].label}
									</h3>
									<p className={`mt-1 text-sm leading-5 text-gray-500`}>
										{GlobalLabels[0].description}
									</p>
								</div>
								<div
									...
									<div className={`sm:col-span-6`}>
										<div className={`relative flex items-center`}>
												...
												<span
													...
													className={`${
														!user.settings.disableLocalTime
															? 'bg-indigo-600'
															: 'bg-gray-200'
													} relative inline-flex flex-shrink-0 h-6 w-10 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:shadow-xs-outline`}
											...
											<div className={`ml-2 pl-12 text-sm leading-5`}>
												...
												>
													{GlobalLabels[1].label}
												</label>
												<p className={`text-gray-500`}>
													{GlobalLabels[1].description}
												</p>
											</div>
										</div>
									</div>
