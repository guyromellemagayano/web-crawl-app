import { MemoizedSeoStatsSkeleton } from "@components/skeletons/SeoStats";
import { SeoChartContents } from "@constants/ChartContents";
...
const SeoStats = () => {
	...
	const seoText = t("sites:sitesSeo");
	const detailsText = t("sites:details");
	const seoErrors = t("sites:seoErrors");
	...
	const totalPagesDuplicatedDescription = stats?.data?.num_pages_duplicated_description ?? 0;
	const totalPagesSeoOk = stats?.data?.num_pages_seo_ok ?? 0;

	// Chart content labels
	const labelsArray = SeoChartContents();
	...
	const chartSeries = [
		...
		totalPagesWithoutH1First,
		totalPagesWithoutH1Second,
		totalPagesWithoutH2First,
		totalPagesWithoutH2Second,
		totalPagesDuplicatedTitle,
		totalPagesDuplicatedDescription,
		totalPagesSeoOk
	];
			...
			<div className="flex justify-between py-8 px-5">
				<div className="flex items-center">
					<h2 className="text-lg font-bold leading-7 text-gray-900">
						{isComponentReady ? seoText : <Skeleton duration={2} width={100} height={15} />}
					</h2>
				</div>
				<div>
					{isComponentReady ? (
						<Link href="/dashboard/sites/[siteId]/pages" as={path} passHref>
...
/**
 * Memoized custom `SeoStats` component
 */
export const MemoizedSeoStats = memo(SeoStats);
