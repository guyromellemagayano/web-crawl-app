import { MemoizedLinksStatsSkeleton } from "@components/skeletons/LinksStats";
import { LinksChartContents } from "@constants/ChartContents";
...
// Dynamic
const Chart = dynamic(() => import("react-apexcharts"), { ssr: false, loading: () => <MemoizedLinksStatsSkeleton /> });

/**
 * Custom function to render the `LinksStats` component
 */
const LinksStats = () => {
	// Translations
	...
	const linkErrorsText = t("sites:linkErrors");
	...
	// Custom variables
	let path = DashboardSitesLink + querySiteId + SiteLinksSlug;

	const totalNonOkLinks = stats?.data?.num_non_ok_links ?? 0;
	const totalNoLinkIssues = stats?.data?.num_ok_links ?? 0;

	// Chart content labels
	const labelsArray = LinksChartContents();
	...
	const chartSeries = [totalNonOkLinks, totalNoLinkIssues];
			...
			<div className="flex justify-between py-8 px-5">
				<div className="flex items-center">
					<h2 className="text-lg font-bold leading-7 text-gray-900">
						{isComponentReady ? linksText : <Skeleton duration={2} width={100} height={15} />}
					</h2>
				</div>
				<div>
					{isComponentReady ? (
...
/**
 * Memoized custom `LinksStats` component
 */
export const MemoizedLinksStats = memo(LinksStats);
