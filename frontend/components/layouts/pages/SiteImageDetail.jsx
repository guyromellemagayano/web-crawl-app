...
/**
 * Custom function to render the `SiteImageDetailPageLayout` component
 */
const SiteImageDetailPageLayout = () => {
	// Translations
	...
	const imageInformationText = t("sites:imageDetail.imageInformation");
	const occurrencesText = t("sites:imageDetail.occurrences");
	const typeText = t("sites:imageDetail.type");
	const urlText = t("sites:imageDetail.url");
	const statusText = t("sites:imageDetail.status");
	const statusAdjustedText = t("sites:imageDetail.statusAdjusted");
	const httpStatusText = t("sites:imageDetail.httpStatus");
	const responseTimeText = t("sites:imageDetail.responseTime");
	const errorText = t("sites:imageDetail.error");
	const sizeText = t("sites:imageDetail.size");
	const tlsStatusText = t("sites:imageDetail.tlsStatus");
	const tlsStatusAdjustedText = t("sites:imageDetail.tlsStatusAdjusted");
	const resolvedStatusText = t("sites:imageDetail.resolvedStatus");
	const resolvedTlsText = t("sites:imageDetail.resolvedTls");
	const missingAltsText = t("sites:imageDetail.missingAlts");
	const missingAltsAdjustedText = t("sites:imageDetail.missingAltsAdjusted");
	const resolvedMissingAltsText = t("sites:imageDetail.resolvedMissingAlts");
	const pageLocationsText = t("sites:imageDetail.pageLocations");
	const visitExternalSiteText = t("sites:visitExternalSite");
	const goToSiteOverviewText = t("sites:goToSiteOverview");
	const altText = t("sites:imageDetail.altText");

	// Custom context
	const { isComponentReady, imageId, user, querySiteId } = useContext(SiteCrawlerAppContext);
	...
	const createdAtTimestamp = imageId?.data?.created_at ?? null;
	const createdAt = createdAtTimestamp
		...
		: null;
	const type = imageId?.data?.type ?? null;
	const status = imageId?.data?.status ?? null;
	const statusAdjusted = imageId?.data?.status_adjusted ?? null;
	const httpStatus = imageId?.data?.http_status ?? null;
	const responseTime = imageId?.data?.response_time ?? null;
	const error = imageId?.data?.error ?? null;
	const size = imageId?.data?.size ?? null;
	const convertedSize = bytes(size ?? 0, {
		thousandsSeparator: " ",
		unitSeparator: " "
	});
	const tlsStatus = imageId?.data?.tls_status ?? null;
	const tlsStatusAdjusted = imageId?.data?.tls_status_adjusted ?? null;
	const missingAlts = imageId?.data?.missing_alts ?? null;
	const missingAltsAdjusted = imageId?.data?.missing_alts_adjusted ?? null;
	const resolvedStatus = imageId?.data?.resolved_status ?? null;
	const resolvedTls = imageId?.data?.resolved_tls ?? null;
	const resolvedMissingAlts = imageId?.data?.resolved_missing_alts ?? null;
	const tls = imageId?.data?.tls ?? null;
	...
	const occurrences = imageId?.data?.pages?.length ?? 0;
	const pages = imageId?.data?.pages ?? null;
		...
		<>
			...
			<div className="flex flex-auto flex-grow flex-col items-center justify-center px-6 pt-8 focus:outline-none sm:px-6 md:px-0">
				<div className="h-full w-full flex-1 items-center justify-center overflow-y-hidden py-4 first-line:flex">
					<div className="pb-12">
						<div>
							<h3 className="text-xl font-bold leading-6 text-gray-900">{imageInformationText}</h3>
						</div>
						<div className="mt-5">
							<dl className="sm:divide-y sm:divide-gray-200">
								...
								<div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
									<dt className="text-sm font-medium text-gray-500">{missingAltsText}</dt>
									{isComponentReady && missingAlts ? (
										<dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{missingAlts}</dd>
									) : isComponentReady && !missingAlts ? (
										<dd className="mt-1 text-sm sm:col-span-2 sm:mt-0">
											<span className="text-gray-500">{noneText}</span>
										</dd>
								...
								<div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
									<dt className="text-sm font-medium text-gray-500">{missingAltsAdjustedText}</dt>
									{isComponentReady && missingAltsAdjusted ? (
										<dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{missingAltsAdjusted}</dd>
									) : isComponentReady && !missingAltsAdjusted ? (
										<dd className="mt-1 text-sm sm:col-span-2 sm:mt-0">
											<span className="text-gray-500">{noneText}</span>
										</dd>
					...
					<div className="pb-12">
						...
						<div className="mt-5">
							<dl className="sm:divide-y sm:divide-gray-200">
								...
								<div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
									<dt className="text-sm font-medium text-gray-500">{resolvedMissingAltsText}</dt>
									{isComponentReady && resolvedMissingAlts ? (
										<dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{resolvedMissingAlts}</dd>
									) : isComponentReady && !resolvedMissingAlts ? (
										<dd className="mt-1 text-sm sm:col-span-2 sm:mt-0">
											<span className="text-gray-500">{noneText}</span>
										</dd>
												...
												{pages.map((page) => (
													<li key={page.id} className="flex items-center justify-between py-3 pl-3 pr-4 text-sm">
														<div className="flex w-0 flex-1 items-start">
															...
															<span className="ml-2 flex w-0 flex-1 flex-col justify-start space-y-1 truncate">
																<span>{page.url}</span>
...
/**
 * Memoized custom `SiteImageDetailPageLayout` component
 */
export const MemoizedSiteImageDetailPageLayout = memo(SiteImageDetailPageLayout);
