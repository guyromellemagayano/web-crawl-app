  ...
  return (
    <PaginationDiv className={`bg-white px-4 pb-4 flex items-center justify-between sm:px-6 align-middle shadow-xs rounded-lg`}>
      <div className={`w-0 flex-1 flex`}>
        {currentPage !== 1 && (
          <Link href='/dashboard/site/[siteId]/links' as={props.pathName + `page=${parseInt(props.page) - 1}`}>
      ...
      <div className={`hidden md:flex`}>
        {pageNumbers.map((val, key) => {
          return (
            <Link key={key} href='/dashboard/site/[siteId]/links' as={props.pathName + 'page=' + val}>
