...
export function PersonalSettingsForm() {
	...
	const profileInformationUpdateSuccess = t("settings:profileInformationUpdate.success");
	const profileInformationUpdateFailed = t("settings:profileInformationUpdate.failed");
		...
		<Formik
			...
			onSubmit={async (values, { setSubmitting }) => {
				const body = {
					...
					large_page_size_threshold: user?.data?.large_page_size_threshold
				};
				...
				if (personalSettingsResponseData !== null && Math.round(personalSettingsResponseStatus / 200) === 1) {
					...
					setSuccessMessage((prevState) => [
						...prevState,
						prevState.indexOf(profileInformationUpdateSuccess) !== -1
							? prevState.find((prevState) => prevState === profileInformationUpdateSuccess)
							: profileInformationUpdateSuccess
					]);
				} else {
					...
					switch (personalSettingsResponseStatus) {
						default:
							errorStatusCodeMessage = profileInformationUpdateFailed;
							break;
					}
