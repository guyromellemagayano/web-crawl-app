	...
	React.useEffect(() => {
		if (Math.round(status / 200) === 1) {
				...
				? (() => {
						switch (status) {
							...
							case 502:
								...
								setTimeout(() => {
									setErrorMessage((prevState) => [...prevState, prevState.indexOf(userBadGatewayError).splice(0, 0)]);
								}, RevalidationInterval);
								break;
							case 503:
							...
							default:
								...
								setTimeout(() => {
									setErrorMessage((prevState) => [...prevState, prevState.indexOf(userUnknownError).splice(0, 0)]);
								}, RevalidationInterval);
								break;
						}
			...
			setTimeout(() => {
				setErrorMessage((prevState) => [
					...prevState,
					prevState.indexOf(userUnknownError) !== -1 ? prevState.splice(prevState.indexOf(userUnknownError), 1) : null
				]);
			}, RevalidationInterval);
		}
