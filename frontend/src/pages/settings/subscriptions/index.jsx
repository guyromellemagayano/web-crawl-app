...
const PaymentMethodForm = loadable(() =>
  import("src/components/forms/PaymentMethodForm")
);
const SiteFooter = loadable(() => import("src/components/footer/SiteFooter"));
const BasicPlan = loadable(() =>
  import("src/components/subscription/BasicPlan")
);
const SemiAnnualPlans = loadable(() =>
  import("src/components/subscription/SemiAnnualPlans")
);
const MonthlyPlans = loadable(() =>
  import("src/components/subscription/MonthlyPlans")
);
...
const Subscriptions = ({ width }) => {
  ...
  const {
    ...
    defaultSubscriptionError: defaultSubscriptionError,
  } = useDefaultSubscription({
    refreshInterval: 1000,
  });
  ...
  useEffect(() => {
    if (
      handleCurrentPaymentPeriod(currentSubscription, currentSubscriptions) > 1
    ) {
          ...
          >
            ...
            <div>
              ...
              <div tw="text-center sm:mt-3">
                <h2
                  tw="mb-3 text-3xl leading-6 font-bold text-gray-900"
                  id="modal-headline"
                >
                    ...
                    <div className="pt-4 m-auto">
                      <h4 className="text-2xl leading-6 font-medium text-gray-900">
                        {pageLoaded ? (
                          ...
                          <Skeleton duration={2} width={150} height={35} />
                        )}
                      </h4>
                    </div>
                  </div>
                    ...
                    <div tw="flex items-center flex-col flex-wrap pt-12 px-4 sm:px-6 lg:px-8 lg:pt-20">
                      <div tw="text-center mb-10">
                        <p tw="text-2xl leading-9 tracking-tight font-bold text-gray-900 sm:text-3xl sm:leading-10">
                          {pageLoaded ? (
                            ...
                            <Skeleton duration={2} width={200} height={35} />
                          )}
                        </p>
                      ...
                      <div tw="mt-10 mb-2">
                        <p tw="text-center text-red-400">
                          {pageLoaded ? (
                            SubscriptionLabel[23].label
                          ) : (
                            <Skeleton duration={2} width={200} height={20} />
                          )}
                        </p>
                      </div>
                    </div>
                            ...
                            {pageLoaded ? (
                              <>
                                {currentSubscriptions &&
                                  currentSubscriptions.results &&
                                  currentSubscriptions.results !== undefined &&
                                  Object.keys(currentSubscriptions.results)
                                    .length > 0 &&
                                  currentSubscriptions.results
                                    .filter(
                                      (result) => result.group.name === "Basic"
                                    )
                                    .map((val, key) => (
                                      <BasicPlan
                                        key={key}
