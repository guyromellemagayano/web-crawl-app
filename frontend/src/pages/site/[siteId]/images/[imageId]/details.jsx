...
const ImagesDetail = () => {
	...
	const { data: imageLocation, error: imageLocationError } = useSWR(
		() => (query.siteId && scanObjId && query.imageId ? imageLocationApiEndpoint : null),
		fetcher
	);
		...
		<Layout>
			{imageLocation && image && site && user ? (
				<Fragment>
					...
					<ImagesDetailDiv className={`h-screen flex overflow-hidden bg-gray-200`}>
								...
								<div className={`max-w-full mx-auto px-4 md:py-4 sm:px-6 md:px-8`}>
									<div>
										<nav className={`sm:hidden`}>
											<Link href="/dashboard/site/[siteId]/images" as={"/dashboard/site/" + query.siteId + "/images"}>
												<a
										...
										<nav className={`hidden sm:flex items-center text-sm leading-5 break-words`}>
											...
											<Link
												href={"/dashboard/site/[siteId]/images/[imageId]/details"}
												as={"/dashboard/site/" + query.siteId + "/images/" + query.imageId + "/details"}
											>
												<a
									...
									<div className={`bg-white shadow overflow-hidden sm:rounded-lg`}>
										<div className={`px-4 py-5 sm:p-0`}>
											<dl>
												<div className={`sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5`}>
													...
													<dd className={`mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2`}>
														{!user.settings.disableLocalTime ? (
															<Fragment>
																<Moment calendar={calendarStrings} date={imageLocation.created_at} local />
																&nbsp;
																<Moment date={imageLocation.created_at} format="hh:mm:ss A" local />
															</Fragment>
														) : (
															<Fragment>
																<Moment calendar={calendarStrings} date={imageLocation.created_at} utc />
																&nbsp;
																<Moment date={imageLocation.created_at} format="hh:mm:ss A" utc />
															</Fragment>
														)}
													</dd>
												</div>
												...
												>
													<dt className={`text-sm leading-5 font-medium text-gray-500`}>Type</dt>
													<dd className={`mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2`}>
												...
												{imageLocation.error !== null && imageLocation.error !== undefined ? (
													<div
														className={`mt-8 sm:mt-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:border-t sm:border-gray-300 sm:px-6 sm:py-5`}
													>
														<dt className={`text-sm leading-5 font-medium text-gray-500`}>Error</dt>
														<dd className={`mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2`}>
										...
										<div
											className={`mt-8 sm:mt-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:border-t sm:border-gray-300 sm:px-6 sm:py-5`}
										>
											<dt className={`text-sm leading-5 font-medium text-gray-500`}>Page Links</dt>
											<dd className={`mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2`}>
