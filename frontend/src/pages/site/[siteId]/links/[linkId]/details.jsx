...
const LinkDetail = () => {
	...
	const { data: link, error: linkError } = useSWR(() => (query.siteId && scanObjId ? scanApiEndpoint : null), fetcher);
	...
	const { data: linkLocation, error: linkLocationError } = useSWR(
		() => (query.siteId && scanObjId && query.linkId ? linkLocationApiEndpoint : null),
		fetcher
	);
		...
		<Layout>
			{link && site && user && linkLocation ? (
				<Fragment>
					...
					<LinkDetailDiv className={`h-screen flex overflow-hidden bg-gray-200`}>
									...
									<div>
										...
										<nav className={`hidden sm:flex items-center text-sm leading-5`}>
											...
											<Link
												...
												as={"/dashboard/site/" + query.siteId + "/links/" + query.linkId + "/details"}
											>
												<a
													className={`font-medium text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out`}
												>
										...
										<div className={`px-4 py-5 sm:p-0`}>
											<dl>
												<div className={`sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5`}>
													...
													<dd className={`mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2`}>
														{!user.settings.disableLocalTime ? (
															<Fragment>
																<Moment calendar={calendarStrings} date={linkLocation.created_at} local />
																&nbsp;
																<Moment date={linkLocation.created_at} format="hh:mm:ss A" local />
															</Fragment>
														) : (
															<Fragment>
																<Moment calendar={calendarStrings} date={linkLocation.created_at} utc />
																&nbsp;
																<Moment date={linkLocation.created_at} format="hh:mm:ss A" utc />
															</Fragment>
														)}
													</dd>
												</div>
												...
												<div
													className={`mt-8 sm:mt-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:border-t sm:border-gray-300 sm:px-6 sm:py-5`}
												>
													<dt className={`text-sm leading-5 font-medium text-gray-500`}>Response Time</dt>
													<dd className={`mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2`}>
