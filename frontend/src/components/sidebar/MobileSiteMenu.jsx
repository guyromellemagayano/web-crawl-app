...
const MobileSiteMenu = (props) => {
	...
	const { data: stats } = useSWR(
		() => (query.siteId && scanObjId ? `/api/site/${query.siteId}/scan/${scanObjId}/` : null),
		getFetcher
	);
	...
	const dropdownHandler = (siteId, verified) => {
		...
		Router.push(
			...
			`/dashboard/site/${siteId}/${pageName}${pageParams !== undefined ? "?" + pageParams : ""}`
		);
	...
	useEffect(() => {
		if (stats && stats.finished_at) props.crawlableHandler !== undefined && props.crawlableHandler(true);
							...
							<>
								...
								<div className="my-3" role="group" aria-labelledby={`${val.slug}-headline`}>
									{val.links.map((val2, key) => {
										const hrefVal =
											val2.url.indexOf("/dashboard/sites") > -1 ? val2.url : `/dashboard/site/[siteId]${val2.url}`;
										const asVal =
							...
							<div className="mt-1" role="group" aria-labelledby={`${val.slug}-headline`}>
								{val.links.map((val2, key) => {
									const hrefVal =
										val2.url.indexOf("/dashboard/sites") > -1 ? val2.url : `/dashboard/site/[siteId]${val2.url}`;
									const asVal =
										val2.url.indexOf("/dashboard/sites") > -1 ? val2.url : "/dashboard/site/" + query.siteId + val2.url;
																...
																>
																	<div className="flex items-center space-x-3">
																		<span
																			aria-label="Online"
																			className={`${
																				selectedSite.verified ? "bg-green-400" : "bg-red-400"
																			} flex-shrink-0 inline-block h-2 w-2 rounded-full`}
																		></span>
																		<span className="block truncate text-gray-600">{selectedSite.name}</span>
																	</div>
																	...
																	>
																		{site.results.map((val, key) => {
																			return (
																				<li
																					key={key}
																					onClick={() => dropdownHandler(val.id, val.verified)}
																					id={`listbox-item-${key}`}
																					role="option"
																					className={`hover:text-white hover:bg-indigo-600 text-gray-900 ${
																						val.verified ? "cursor-pointer" : "cursor-not-allowed"
																				...
																				>
																					<div className="flex items-center space-x-3">
																						<span
																							aria-label="Online"
																							className={`${
																								val.verified ? "bg-green-400" : "bg-red-400"
																							} flex-shrink-0 inline-block h-2 w-2 rounded-full`}
																						></span>
																						<span className="font-normal block truncate">{val.name}</span>
																					</div>
