...
const CurrentStatusStats = ({ sid, uptimeSummary }) => {
	...
	const uptimeCurrent = uptimeSummary?.current;
	const uptimeLastOk = uptimeSummary?.last_ok;
	const uptimeLastDowntime = uptimeSummary?.last_downtime;

	return uptimeCurrent !== undefined &&
		uptimeCurrent !== null &&
		Object.keys(uptimeCurrent).length > 0 ? (
			...
			<div tw="flex justify-between py-8 px-5">
				<div tw="flex items-center">
					...
					<h2 tw="text-lg font-bold leading-7 text-gray-900">
						{componentReady ? (
							CurrentStatusStatsLabels[0].label
						) : (
									...
									<dl tw="relative top-1">
										{uptimeCurrent?.status !== null ? (
											uptimeCurrent?.status == "OK" ? (
												<dt
													tw="inline-flex w-2 h-2 bg-green-400 rounded-full"
													aria-label="uptime-status"
												/>
											) : (
												<dt
													tw="inline-flex w-2 h-2 bg-red-400 rounded-full"
													aria-label="uptime-status"
												/>
											)
										) : (
