...
const DataTable = (props) => {
	...
	const scanApiEndpoint = "/api/site/" + props.site.id + "/scan/?ordering=-finished_at";
	...
	const { data: scan } = useSWR(() => (props ? scanApiEndpoint : null), useFetcher);
	...
	return scan && stats ? (
		<DataTableDiv>
					...
					{props.site.url && props.site.name ? (
						<div tw="mr-4">
							<div tw="truncate text-sm leading-5 font-medium text-gray-900">
								{!props.site.verified ? (
									<span tw="text-sm leading-5 font-semibold text-gray-500">{props.site.name}</span>
								) : (
									<Link href="/site/[siteId]/overview" as={`/site/${props.site.id}/overview`}>
			...
			<td tw="px-6 py-4 whitespace-nowrap border-b border-gray-300">
				{stats["verified"] === undefined ? (
					...
					<div tw="text-sm leading-5 text-gray-500">{DataTableLabel[2].label}</div>
				)}
			</td>
			{setTotalIssues() > 0 ? (
				<td tw="px-6 py-4 whitespace-nowrap border-b border-gray-300 text-sm leading-5 font-semibold text-red-500">
					<Link href="/site/[siteId]/overview" as={`/site/${props.site.id}/overview`} passHref>
			...
			{stats.num_links ? (
				<td tw="px-6 py-4 whitespace-nowrap border-b border-gray-300 text-sm leading-5 font-semibold text-gray-500">
					<Link href="/site/[siteId]/links" as={`/site/${props.site.id}/links`} passHref>
			...
			{stats.num_pages ? (
				<td tw="px-6 py-4 whitespace-nowrap border-b border-gray-300 text-sm leading-5 font-semibold text-gray-500">
					<Link href="/site/[siteId]/pages" as={`/site/${props.site.id}/pages`} passHref>
						<a tw="cursor-pointer text-sm leading-6 font-semibold text-indigo-600 hover:text-indigo-500 transition ease-in-out duration-150">
							{stats.num_pages}
						</a>
					</Link>
			...
			{stats.num_images ? (
				<td tw="px-6 py-4 whitespace-nowrap border-b border-gray-300 text-sm leading-5 font-semibold text-gray-500">
					<Link href="/site/[siteId]/images" as={`/site/${props.site.id}/images`} passHref>
						<a tw="cursor-pointer text-sm leading-6 font-semibold text-indigo-600 hover:text-indigo-500 transition ease-in-out duration-150">
							{stats.num_images}
						</a>
					</Link>
		...
		<DataTableDiv>
			{[...Array(6)].map((val, key) => (
				<td tw="flex-none px-6 py-4 whitespace-nowrap border-b border-gray-300" key={key}>
					<Skeleton duration={2} />
				</td>
			))}
		</DataTableDiv>
