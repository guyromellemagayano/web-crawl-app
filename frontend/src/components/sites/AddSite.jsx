...
const AddSite = ({ searchKey, onSearchEvent }) => {
	...
	const { data: sites } = useSWR(sitesApiEndpoint, useFetcher);
	...
	return sites && user ? (
		<>
			...
			<AddSiteDiv className="py-4">
				<div className="px-4 py-5 border-b border-gray-300 sm:px-6 bg-white overflow-hidden rounded-lg sm:shadow-xs">
							...
							<span className="inline-flex rounded-md shadow-xs-sm">
								...
								siteLimitCounter > maxSiteLimit ? (
									...
									<Link href="/dashboard/sites/information">
										<a className="relative inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-green-600 hover:bg-green-500 focus:outline-none focus:shadow-xs-outline-green focus:border-green-700 active:bg-green-700">
											{AddSiteLabel[0].label}
										</a>
									</Link>
								)}
...
AddSite.propTypes = {
	siteLimitCounter: PropTypes.number,
	maxSiteLimit: PropTypes.number,
