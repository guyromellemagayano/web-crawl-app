...
import StylesheetTableContent from 'public/data/stylesheet-table.json'
import useUser from 'hooks/useUser'
import StylesheetTable from 'components/site/StylesheetTable'
...
const Stylesheets = props => {
	...
	let scanApiEndpoint = props.result.page !== undefined ? `/api/site/${query.siteId}/scan/${scanObjId}/stylesheet/?page=` + props.result.page : `/api/site/${query.siteId}/scan/${scanObjId}/stylesheet/`
	...
	const { data: stylesheet, error: stylesheetError, mutate: updateLinks } = useSWR(
	...
	const searchEventHandler = async (e) => {
		...
		if(e.target.value == '' || e.target.value == ' ') {
			...
			Router.push('/dashboard/site/[siteId]/stylesheets', newPath)
			return
		}
	...
	{stylesheetError && <Layout>{stylesheetError.message}</Layout>}
	{scanError && <Layout>{scanError.message}</Layout>}
	{siteError && <Layout>{siteError.message}</Layout>}

	return (
		<Layout>
		  {user && stylesheet && site ? (
			<Fragment>
			  <Head>
				<title>{pageTitle} {site.name}</title>
			  </Head>
	
			  <StylesheetsDiv className={`h-screen flex overflow-hidden bg-gray-100`}>
				...
				<div className={`flex flex-col w-0 flex-1 overflow-hidden`}>
				  ...
				  >
					<div className={`max-w-full mx-auto px-4 md:py-4 sm:px-6 md:px-8`}>
					  <div>
						...
						<nav className={`hidden sm:flex items-center text-sm leading-5`}>
						  ...
						  <Link href="/dashboard/site/[siteId]/stylesheets" as={'/dashboard/site/' + query.siteId + '/stylesheets'}>
							<a className={`font-medium text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out`}>All Stylesheets</a>
						  </Link>
						</nav>
					  </div>
					...
					<div className={`max-w-full mx-auto px-4 sm:px-6 md:px-8`}>
					  ...
					  <StylesheetFilter onFilterChange={filterChangeHandler} allFilter={allFilter} issueFilter={issueFilter} internalFilter={internalFilter} externalFilter={externalFilter} />
					  <div className={`pb-4`}>
						  ...
						  >
							...
							>
							  <table className={`min-w-full`}>
								<thead>
								  <tr>
									{StylesheetTableContent.map((site, key) => {
									  return (
										<Fragment key={key}>
										  <th
										  ...
										  >
											<div className={`flex items-center justify-start`}>
											  <StylesheetSortingDiv sortOrder={sortOrder} onSortHandler={SortHandler} key={key} slug={site.slug} />
								...
								{stylesheet.results && stylesheet.results.map((val, key) => (
									<StylesheetTable key={key} val={val} />
								))}
							  </table>
							</div>
						  </div>
					  ...
					  <Pagination 
						href='/dashboard/site/[siteId]/stylesheets'
						pathName={pagePath}
