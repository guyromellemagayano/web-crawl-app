...
const StylesheetDetailDiv = styled.div``

const StylesheetDetail = () => {
  ...
  const linkLocationApiEndpoint = `/api/site/${query.siteId}/scan/${scanObjId}/stylesheet/${query.stylesheetId}/`

  const { data: linkLocation, error: linkLocationError } = useSWR(
    () => query.siteId && scanObjId && query.stylesheetId ? linkLocationApiEndpoint : null, fetcher
  )
  ...
  const pageTitle = `Stylesheet Detail | ${linkLocation.url}`
    ...
    <Layout>
      ...
      <StylesheetDetailDiv className={`h-screen flex overflow-hidden bg-gray-100`}>
...
export default StylesheetDetail