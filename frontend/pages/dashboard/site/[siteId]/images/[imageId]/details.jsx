...
const ImagesDetail = () => {
  ...
  const { data: imageLocation, error: imageLocationError } = useSWR(
    () => query.siteId && scanObjId && query.imageId ? imageLocationApiEndpoint : null, fetcher
  )
	...
	return (
		<Layout>
			{imageLocation && image && site && user ? (
				<Fragment>
					<Head>
						<title>{imageTitle} {imageLocation.url}</title>
					</Head>

					<ImagesDetailDiv className={`h-screen flex overflow-hidden bg-gray-100`}>
                ...
                <div className={`max-w-full mx-auto px-4 md:py-4 sm:px-6 md:px-8`}>
                  <div>
                    ...
                    <nav className={`hidden sm:flex items-center text-sm leading-5 break-words`}>
                      ...
                      <Link href='/dashboard/site/[siteId]/images' as={'/dashboard/site/' + query.siteId + '/images'}>
                        <a className={`whitespace-no-wrap font-medium text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out`}>All images</a>
                      </Link>
                      <svg className={`flex-shrink-0 mx-2 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor`}>
                        <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"/>
                      </svg>
