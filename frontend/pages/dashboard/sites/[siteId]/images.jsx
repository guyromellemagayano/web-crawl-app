...
const SiteImagesAuth = ({ siteName }) => {
	...
	return isComponentReady && Math.round(user?.status / 100) === 2 && !user?.data?.detail ? (
		<MemoizedLayout>
			...
			<MemoizedPageLayout pageTitle={sitesImagesText}>
...
export default function SiteImages({ siteName, fallback }) {
	return (
		<SWRConfig value={{ fallback }}>
			<SiteImagesAuth siteName={siteName} />
		</SWRConfig>
	);
}
